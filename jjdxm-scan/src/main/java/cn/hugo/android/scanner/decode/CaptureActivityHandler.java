...
public final class CaptureActivityHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        int id = message.what;
        if (id == ResourceUtils.getResourceIdByName(activity, "id", "restart_preview")) { // 准备进行下一次扫描
            Log.d(TAG, "Got restart preview message");
            restartPreviewAndDecode();
        } else if (id == ResourceUtils.getResourceIdByName(activity, "id", "decode_succeeded")) {
            ...
            activity.handleDecode((Result) message.obj, barcode,
                    scaleFactor);
        } else if (id == ResourceUtils.getResourceIdByName(activity, "id", "decode_failed")) {
            // We're decoding as fast as possible, so when one decode fails,
            // start another.
            state = State.PREVIEW;
            cameraManager.requestPreviewFrame(decodeThread.getHandler(),
                    ResourceUtils.getResourceIdByName(activity, "id", "decode"));
        } else if (id == ResourceUtils.getResourceIdByName(activity, "id", "return_scan_result")) {
            Log.d(TAG, "Got return scan result message");
            activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
            activity.finish();
        } else if (id == ResourceUtils.getResourceIdByName(activity, "id", "launch_product_query")) {
    ...
    public void quitSynchronously() {
        ...
        Message quit = Message.obtain(decodeThread.getHandler(), ResourceUtils.getResourceIdByName(activity, "id", "quit"));
        quit.sendToTarget();
        ...
        removeMessages(ResourceUtils.getResourceIdByName(activity, "id", "decode_succeeded"));
        removeMessages(ResourceUtils.getResourceIdByName(activity, "id", "decode_failed"));
    }
